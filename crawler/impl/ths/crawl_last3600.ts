/**
 * 从爱问财打开页面 http://www.iwencai.com/unifiedwap/result?w=600000 收集的数据请求地址。
 */
...
const crawl: KCrawler = async (code: string, period = KPeriod.Day, debug = false): Promise<StockKData> => {
  ...
  const url = `http://d.10jqka.com.cn/v6/line/${type}_${code}/${sp}/last3600.js?ts=${Date.now()}`
  ...
  /**
   * 响应体数据结构例子: quotebridge_v6_line_17_000158_00_last3600({
   *   "data":"yyyyMMdd,open,high,low,close,vol,amo,,期货昨日结算价,,0;...",
